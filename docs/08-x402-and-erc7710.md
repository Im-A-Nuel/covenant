...
permissions." Covenant fuses them. The **same USDC transfer** that satisfies x402's *"prove you paid"*
is the very transfer **constrained by the ERC-7710 caveats**. There is exactly one on-chain action, and
it is simultaneously the payment and the thing the covenant guards.
...
sequenceDiagram
  ...
  X402-->>Agent: 402 { accepts: [{ asset: USDC, maxAmountRequired, payTo, ... }] }
  ...
  Agent->>DM: redeemDelegations([{ permissionContext: [delegation],<br/>executions: [transfer(payTo, maxAmountRequired)] }])
...
design where the payment rail and the permission system are the same motion. See
[10 · Hackathon tracks](./10-hackathon-tracks.md).

---

**Next:** [09 · Demo guide →](./09-demo-guide.md)
