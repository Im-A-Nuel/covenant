...
> is built to be truthful about what is real (see [Real vs Simulated](../core-concepts/real-vs-simulated.md)),
...
| --- | --- | --- |
| **Session-only delegations** | Signed delegations live in a runtime ref, not `localStorage` ([why](../architecture/technical-reference.md#state--persistence)). | A real on-chain redemption only works in the session that created the covenant. |
...
* **Automated test suite.** Unit tests for `evaluatePolicy` (the decision matrix), integration tests for
  the 402 → redeem → verify loop, and a forked-chain test for caveat enforcement.
...
* **Claiming on-chain settlement when it isn't.** The app will always badge `simulated` honestly rather
  than fake a hash. (See [Real vs Simulated](../core-concepts/real-vs-simulated.md).)
